...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            XYPhotoView *photoView = [[XYPhotoView alloc] init];
...
{
    unsigned long count = self.photos.count;
    ...
    for (int i = 0; i<count; i++) {
        ...
        XYPhoto *XYphoto = self.photos[i];
        NSString *photoUrl = [XYphoto.thumbnail_pic stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        XYPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (XYPhotoW + XYPhotoMargin);
            CGFloat photoY = row * (XYPhotoH + XYPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, XYPhotoW, XYPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * XYPhotoH + (rows - 1) * XYPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * XYPhotoW + (cols - 1) * XYPhotoMargin;
